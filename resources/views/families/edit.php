...
<form method="post" action="/families/<?= (int) ($family['id'] ?? 0) ?>">
    <div>
        ...
        <select id="status_id" name="status_id" required>
            ...
            <?php foreach ($statuses as $status): ?>
                ...
                <option value="<?= htmlspecialchars((string) $statusId, ENT_QUOTES, 'UTF-8') ?>" <?= $isSelected($family['status_id'] ?? null, $statusId) ? 'selected' : '' ?>>
    ...
    <div>
        ...
        <input id="family_code" name="family_code" type="text" maxlength="30" required value="<?= htmlspecialchars((string) ($family['family_code'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
    </div>

    <div>
        <label for="name">Name</label>
        <input id="name" name="name" type="text" maxlength="150" value="<?= htmlspecialchars((string) ($family['name'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
    </div>

    <div>
        <label for="notes">Notes</label>
        <textarea id="notes" name="notes"><?= htmlspecialchars((string) ($family['notes'] ?? ''), ENT_QUOTES, 'UTF-8') ?></textarea>
    </div>
