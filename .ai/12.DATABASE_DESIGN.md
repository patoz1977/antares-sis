...
**Propósito:** dirección reutilizable de una Family. Columnas: `id BIGINT UNSIGNED PK`, `family_id BIGINT UNSIGNED NOT NULL FK families.id`, `label VARCHAR(100) NOT NULL`, `province_id`, `canton_id`, `parish_id` BIGINT UNSIGNED NOT NULL FK, `main_street VARCHAR(150) NOT NULL`, `street_number VARCHAR(30) NULL`, `secondary_street VARCHAR(150) NULL`, `sector VARCHAR(150) NULL`, `reference VARCHAR(255) NULL`, `latitude DECIMAL(10,7) NULL CHECK(latitude BETWEEN -90 AND 90)`, `longitude DECIMAL(10,7) NULL CHECK(longitude BETWEEN -180 AND 180)`, `status_id BIGINT UNSIGNED NOT NULL FK statuses.id` y auditoría estándar. UNIQUE `(family_id,label,deleted_at)`; índices de FKs.

# Tablas: representative_address_assignments y student_address_assignments

Asignaciones físicas no polimórficas. Ambas incluyen `id BIGINT UNSIGNED PK`, `family_address_id BIGINT UNSIGNED NOT NULL FK family_addresses.id`, miembro FK, `ended_at TIMESTAMP NULL`, auditoría. `representative_address_assignments` incluye `representative_id BIGINT UNSIGNED NOT NULL` y `active_representative_id BIGINT UNSIGNED GENERATED ALWAYS AS (IF(ended_at IS NULL, representative_id, NULL)) STORED`, UNIQUE `(active_representative_id)`. `student_address_assignments` incluye `student_id BIGINT UNSIGNED NOT NULL` y `active_student_id BIGINT UNSIGNED GENERATED ALWAYS AS (IF(ended_at IS NULL, student_id, NULL)) STORED`, UNIQUE `(active_student_id)`. MySQL 8 aplica esos índices únicos; el servicio valida misma Family dentro de la transacción.
...
`emergency_contact_assignments`: `id PK`, `family_emergency_contact_id FK`, `student_id FK`, `priority TINYINT UNSIGNED NULL`, `status_id FK`, auditoría; UNIQUE `(family_emergency_contact_id,student_id,deleted_at)`. Contacto y Student se validan en la misma Family.
...
`authorized_pickup_assignments`: `id PK`, `family_authorized_pickup_id FK`, `student_id FK`, `status_id FK`, auditoría; UNIQUE `(family_authorized_pickup_id,student_id,deleted_at)`.

# Tabla: enrollments

`id BIGINT UNSIGNED PK`; `student_id`, `family_id`, `academic_period_id`, `grade_id` NOT NULL FK; `section_id FK NULL`; BillingInformation: `billing_responsible_name VARCHAR(200) NOT NULL`, `billing_identification VARCHAR(50) NOT NULL`, `billing_email VARCHAR(254) NULL`, `billing_phone VARCHAR(30) NULL`; `medical_information TEXT NULL`; `requires_institutional_transport TINYINT(1) NOT NULL CHECK (requires_institutional_transport IN (0,1))`; `is_authorized_to_leave_alone TINYINT(1) NOT NULL CHECK (is_authorized_to_leave_alone IN (0,1))`; `status_id FK`; `started_at TIMESTAMP NOT NULL`, lifecycle timestamps y auditoría. UNIQUE `(student_id,academic_period_id,deleted_at)`; índices de FKs. Estados: Draft, Submitted, Completed y Cancelled.

# Documentos y snapshot

`institutional_documents(id PK,name VARCHAR(200),description TEXT NULL,status_id FK,auditoría)`; `institutional_document_versions(id PK,institutional_document_id FK,version VARCHAR(50),file_reference VARCHAR(500),published_at TIMESTAMP,status_id FK,UNIQUE(document_id,version,deleted_at))`; `document_requirements(id PK,institutional_document_version_id FK,academic_period_id FK,is_required TINYINT(1),UNIQUE(version_id,period_id,deleted_at))`; `document_acceptances(id PK,enrollment_id FK,institutional_document_version_id FK,accepted_by_representative_id FK,accepted_at TIMESTAMP,UNIQUE(enrollment_id,version_id,deleted_at))`.
