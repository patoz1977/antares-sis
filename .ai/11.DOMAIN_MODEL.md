...
User no existe sin Person; una Person puede existir sin User. Para Representative, LoginIdentifier es el DocumentNumber actual de Person y su valor normalizado es globalmente único dentro de la base institucional. Cambiarlo conserva UserId, PersonId, PasswordHash, UserStatus, failed-login state y relaciones. Nunca se conserva una contraseña reversible. La creación y el cambio administrativo reciben el texto únicamente durante la operación, exigen al menos cinco caracteres, permiten letras y/o números sin mezcla, mayúscula o símbolo obligatorios y no imponen cambio en el primer login. Student no tiene User ni Portal en el MVP.
...
Los cambios internos de credencial y estado de acceso se confirman dentro de User. La sincronización DocumentNumber/LoginIdentifier involucra Person y User, pertenece a Application, usa una sola transacción exterior y revierte ambos Aggregates ante cualquier fallo; no se implementa con SQL ad hoc ni desde PersonRepository.
...
Un Representative puede pertenecer a varias Family. Su Person debe conservar siempre un email personal válido, exista o no Representative User; WorkEmail permanece opcional y no satisface esta regla. El acceso al portal requiere un User activo que pertenece exactamente a su Person, no una duplicación personal. Los campos reservados solo los administra Secretaría.
