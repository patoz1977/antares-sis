...
The E015 workbook contains personal data and an initial Representative password and is therefore sensitive untrusted input. Its later implementation must satisfy all of the following without weakening the general file rules:
...
Phase 2 defined these boundaries without implementation. Phase 5 implements the pinned reviewed OpenSpout XLSX adapter against a verified local path and the fail-closed ZIP/OOXML, formula, structure, type and resource-limit validation boundary; its Composer advisory check reports no known advisories. Phase 6 implements read-only password-free safe Preview and complete current-input Apply reparse/revalidation, clears every plaintext password wrapper after processing and returns no raw row, email, birth date, physical ID, SQL or exception detail. Temporary upload ownership, actor/session-bound token and digest enforcement and formula-neutralized downloadable error CSV remain Phase 7 Delivery responsibilities and are not implemented by Phase 6.
