...
Phase 2 defines these boundaries but implements none of them. The future parser must use the pinned reviewed OpenSpout XLSX adapter against a verified local path, run Composer advisory checks when the dependency is added, and neutralize formula-trigger prefixes in downloadable error CSV.
