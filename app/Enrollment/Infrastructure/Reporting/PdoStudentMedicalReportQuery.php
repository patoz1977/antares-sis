...
final class PdoStudentMedicalReportQuery extends PdoEnrollmentReportingQuery implements StudentMedicalReportQuery
{
    public function fetch(int $academicPeriodId): array
    {
        ...
        $rows = $this->rows(
            ...
            . 'e.has_medical_condition, e.medical_condition_detail, e.has_allergies, e.allergy_detail, '
        ...
        foreach ($rows as $row) {
            ...
            if (isset($seen[$studentId])) {
                throw new RuntimeException('Medical report returned duplicate Student rows.');
            }
            $seen[$studentId] = true;
            ...
            $medical = $this->medicalInformation($row);
            if (!$active) {
                continue;
            }

            $result[] = new StudentMedicalReportRow(
                $studentId,
                $gradeId,
                $gradeName,
                $sectionId,
